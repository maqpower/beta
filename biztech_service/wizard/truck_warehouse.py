...
class TruckWarehouseLine(models.TransientModel):
    _name = "truck.warehouse.wiz.line"

    warehouse_service_id=fields.Many2one('truck.warehouse.wiz',string="Fleet Service")
...
class TruckWarehouseWiz(models.TransientModel):
    _name = 'truck.warehouse.wiz'

    warehouse_location_ids = fields.One2many('truck.warehouse.wiz.line','warehouse_service_id',string="Fleet Location")
    ...
    @api.onchange('warehouse_location_ids')
    @api.multi
    def onchnage_truck_button(self):
        for line in self.warehouse_location_ids:
    ...
    def onchnage_inter_truck_product(self):
        ...
        for stocks in stock_quants_obj:
            if stocks.location_id.is_fleet_location:
                ...
                if qty >0 :
                    truck_list.append((0,0,{'location_id':stocks.location_id.id,'qty':qty,'final_quantity':0.0,'dest_location_id':stock_location_id}))
                    self.warehouse_location_ids = truck_list
            else:
                self.warehouse_location_ids = truck_list

    #Internal Truck Transfer Move
    @api.multi
    def truck_transfer_warehouse(self):
        for picking in self:
            for line in picking.warehouse_location_ids:
                if line.is_truck:
                    move_line_list = []
                    context = self.env.context
                    active_id = context.get('active_id')
                    truck_id = self.env['stock.picking.type'].search([('name','=','Receipts')])
